import { zodResolver } from '@hookform/resolvers/zod';
import { getRouteApi } from '@tanstack/react-router';
import { Controller, useForm, type SubmitHandler } from 'react-hook-form';
import { useTranslation } from 'react-i18next';
import * as z from 'zod/v4';
import { proficiencyLevels } from '~/app';
import { useUpdateProfileLanguage } from '~/features';
import { Button, Select, SelectItem, type LanguageProficiency } from '~/shared';

const routeApi = getRouteApi('/_mainLayout/users/$userId/_userLayout/languages');

const updateLanguageSchema = z.object({
  proficiency: z.enum(proficiencyLevels),
});

type UpdateLanguageForm = z.infer<typeof updateLanguageSchema>;

type UpdateLanguageFormProps = { language: LanguageProficiency; onSuccess: () => void; onCancel: () => void };

export const UpdateLanguageForm: React.FC<UpdateLanguageFormProps> = (props) => {
  const { language, onSuccess, onCancel } = props;
  const { t } = useTranslation();
  const params = routeApi.useParams();
  const form = useForm<UpdateLanguageForm>({
    resolver: zodResolver(updateLanguageSchema),
    defaultValues: {
      proficiency: language.proficiency,
    },
  });
  const { updateProfileLanguage } = useUpdateProfileLanguage({
    onSuccess,
  });
  const labelProps = {
    className: 'bg-bg dark:bg-neutral-600',
  };

  const handleSubmit: SubmitHandler<UpdateLanguageForm> = (data) => {
    updateProfileLanguage({
      userId: params.userId,
      language: { name: language.name, proficiency: data.proficiency },
    });
  };

  return (
    <form className="flex flex-col gap-8 mt-4" onSubmit={form.handleSubmit(handleSubmit)}>
      <div className="flex flex-col gap-4">
        <Select label={t('Language')} className="w-full" labelProps={labelProps} value={language.name} disabled>
          <SelectItem value={language.name}>{language.name}</SelectItem>
        </Select>

        <Controller
          name="proficiency"
          control={form.control}
          render={({ field }) => (
            <Select label={t('Language proficiency')} className="w-full" labelProps={labelProps} {...field}>
              {proficiencyLevels.map((pl) => (
                <SelectItem key={pl} value={pl}>
                  {t(pl)}
                </SelectItem>
              ))}
            </Select>
          )}
        />
      </div>

      <div className="flex gap-2 justify-end">
        <Button variant="outlined" onClick={() => onCancel()}>
          {t('Cancel')}
        </Button>
        <Button type="submit">{t('Confirm')}</Button>
      </div>
    </form>
  );
};
